...
def monitor():
    ...
    if file_monitor:
        ...
        input("[INFO] Press ENTER to continue...")
        dg_global.flag = True
    return monitor_unix_time, monitor_reltime, resprate, heartrate, satrate
...
def sensor():
    ...
    root = tk.Tk()
    root.withdraw()
    file_sensor = filedialog.askopenfilename(filetypes=[('.textfiles', '.txt')], title='Select sensor.txt')
    root.destroy()
    ...
    if file_sensor:
        ...
        input("[INFO] Press ENTER to continue...")
        dg_global.flag = True
    return hertz, hourshex, minuteshex, secondshex, millsechex, accx1hex, accx2hex, accy1hex, accy2hex, accz1hex, accz2hex, gyrox1hex, gyrox2hex, gyroy1hex, gyroy2hex, gyroz1hex, gyroz2hex